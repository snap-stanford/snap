...
protected:
  ...
  bool shared_memory; // True if the vector array is in shared memory
...
public:
  TVec(): MxVals(0), Vals(0), ValT(NULL), shared_memory(false) {}
  ...
  explicit TVec(TVal *_ValT, const TSizeTy& _Vals):
    MxVals(-1), Vals(_Vals), ValT(_ValT), shared_memory(false){}
  ~TVec() {
    if ((ValT!=NULL) && (MxVals!=-1)) {
      delete[] ValT;
    }
  }
  explicit TVec(TSIn& SIn): MxVals(0), Vals(0), ValT(NULL), shared_memory(false) {Load(SIn);}
  ...
  void LoadShM(TShMIn& ShMin, BinaryFunction loadFromShared) {
    if ( (ValT!=NULL) && (MxVals!=-1)){
      delete[] ValT;
    }
    ShMin.Load(MxVals);
    ...
    if (MxVals == 0) {
      ValT = NULL;
        ...
        for (TSizeTy ValN=0; ValN<Vals; ValN++) {
          loadFromShared(ValT+ValN, ShMin);
        }
    }
    shared_memory = false;
  }
  ...
  void SetVal(const TSizeTy& ValN, const TVal& Val){
    IAssertR(!(shared_memory && (MxVals == -1)), "Cannot directly access shared memory");
  ...
  void Swap(const TSizeTy& ValN1, const TSizeTy& ValN2){IAssertR(!(shared_memory && (MxVals == -1)), "Cannot directly access shared memory");
...
void TVec<TVal, TSizeTy>::Resize(const TSizeTy& _MxVals){
  IAssertR(MxVals!=-1 || shared_memory, TStr::Fmt("Can not increase the capacity of the vector. %s. [Program failed to allocate more memory. Solution: Get a bigger machine and a 64-bit compiler.]", GetTypeNm(*this).CStr()).CStr());
...
void TVec<TVal, TSizeTy>::Clr(const bool& DoDel, const TSizeTy& NoDelLim){
  if ((DoDel)||((!DoDel)&&(NoDelLim!=-1)&&(MxVals>NoDelLim))){
    ...
    IAssertR(MxVals!=-1 || shared_memory, "This vector was obtained from TVecPool. Such vectors cannot change its size!");
    Vals=0;
  }
}
...
void TVec<TVal, TSizeTy>::Trunc(const TSizeTy& _Vals){
  IAssertR(!(MxVals==-1 && shared_memory), "Cannot truncate a shared memory vector");
...
void TVec<TVal, TSizeTy>::Pack(){
  IAssertR(!(shared_memory && (MxVals == -1)), "Cannot pack accessed shared memory");
  IAssertR(MxVals!=-1, "This vector was obtained from TVecPool. Such vectors cannot change its size!");
  if (Vals==0){
...
TSizeTy TVec<TVal, TSizeTy>::AddV(const TVec<TVal, TSizeTy>& ValV){
  AssertR(MxVals!=-1 || shared_memory, "This vector was obtained from TVecPool. Such vectors cannot change its size!");
