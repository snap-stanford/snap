...
inline void ToNetworkCrossMP(PGraphMP Curr, TCrossNet& Cross,
  ...
  Sw->Start(TStopwatch::AllocateColumnCopies);
  const TInt NumRows = Cross.GetEdges();
  ...
  if (NodeType == atInt) {
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < Partitions.Len(); i++) {
      TCrossEdgeI EI = Cross.GetCrossEdgeI(Partitions[i].GetVal1());
      TCrossEdgeI EndI = Cross.GetCrossEdgeI(Partitions[i].GetVal2());
      while (EI < EndI) {
        TInt EId = EI.GetId();
        SrcCol1[EId] = EI.GetSrcNId();
        EdgeCol1[EId] = EId;
        DstCol2[EId] = EI.GetDstNId();
        EdgeCol2[EId] = EId;
        EI++;
      }
    }
