...
float Equation(PUNGraph& Graph, TIntFltH& PAlpha,float& SumPAlphaLogPAlpha, TIntFltH& Qi){
  ...
  for (int i=0; i<Qi.Len(); i++) {
    SumQi += static_cast<float>(Qi[i]);
    SumQiLogQi += static_cast<float>(Qi[i]*log(Qi[i]));
    SumQiSumPAlphaLogQiSumPAlpha += static_cast<float>((Qi[i]+SumPAlpha)*log(Qi[i]+SumPAlpha));
  }
