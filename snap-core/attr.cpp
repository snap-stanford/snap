/// Add functions
int TSparseAttrSingle::AddSAttrDat(const int& Id, const TStr& AttrName, TInt& Val) {
  if (!AttrNameToId.IsKey(AttrName)) { return -1; }
  TInt AttrId = AttrNameToId.GetDat(AttrName).GetVal2();
  return AddSAttrDat(Id, AttrId, Val);
} 
int TSparseAttrSingle::AddSAttrDat(const int& Id, const TInt& AttrId, TInt& Val) {
...
/// Get functions
int TSparseAttrSingle::GetSAttrDat(const int& Id, const TStr& AttrName, TInt& Val) const {
  if (!AttrNameToId.IsKey(AttrName)) { return -1; }
  TInt AttrId = AttrNameToId.GetDat(AttrName).GetVal2();
  return GetSAttrDat(Id, AttrId, Val);
} 
int TSparseAttrSingle::GetSAttrDat(const int& Id, const TInt& AttrId, TInt& Val) const {
  TPair<TInt, TInt> Key(AttrId, Id);
...
/// Del functions
int TSparseAttrSingle::DelSAttrDat(const int& Id, const TStr& AttrName) {
  if (!AttrNameToId.IsKey(AttrName)) { return -1; }
  TInt AttrId = AttrNameToId.GetDat(AttrName).GetVal2();
  return DelSAttrDat(Id, AttrId);
} 
int TSparseAttrSingle::DelSAttrDat(const int& Id, const TInt& AttrId) {
  TPair<TInt, TInt> Key(AttrId, Id);
  if (IntAttrVals.IsKey(Key)) {
