...
void TTimeNet::PlotCCfOverTm(const TStr& FNmPref, TStr Desc, const TTmUnit& TmUnit, const int& NodesBucket) const {
  ...
  for (int t = 0; t < TmBucketV.Len(); t++) {
    ...
    OpClV.Add(TFltPr(XVal, static_cast<double>(Open+Close==0?0:Close/(Open+Close))));
    OpV.Add(TFltPr(XVal, static_cast<double>(Open==0?0:Close/Open)));
    Tuple[0]=Graph->GetNodes();
    Tuple[1]=Graph->GetEdges();
    Tuple[2]=static_cast<double>(Close);  Tuple[3]=static_cast<double>(Open);
    OpenClsV.Add(Tuple);
