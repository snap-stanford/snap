...
PNGraph CascGraph(PTable P,const TStr C1,const TStr C2,const TStr C3,const TStr C4,const TInt W) {
    ...
    for(int i = BIdx; i < Start.Len(); i++) {
      ...
      if (InSource == ODest && InStart >= (ODur + OStart)) {
        if (!Graph->IsEdge(OIdx,InIdx)) {
          int n = Graph->AddEdge(OIdx,InIdx);
        }
      }      
    }
  }
...
#ifdef USE_OPENMP
void CascFindMP(PNGraph Graph,PTable P,const TStr C1,const TStr C2,const TStr C3,const TStr C4,TVec<TIntV> &TopCascVV,bool Print = true) {
  // Attribute to Int mapping
  TInt SIdx = P->GetColIdx(C1);
  TInt DIdx = P->GetColIdx(C2);
  TInt StIdx = P->GetColIdx(C3);
  ...
  #pragma omp parallel private(ThTopCascVV) num_threads(10)
  {
    ...
    for (int i = 0; i < GNodeV.Len(); i++) {
      TIntV CurCasc;
      TSnapQueue<TInt> EventQ;
      //THashSet<TInt> VisitedH(P->GetNumRows().Val);
      THashSet<TInt> VisitedH;
      TInt NId = GNodeV[i];
      EventQ.Push(NId);
      VisitedH.AddKey(NId);
