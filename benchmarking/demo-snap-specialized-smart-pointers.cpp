...
class RingoTable : public TVec<TPt<PVec<TInt> > > {
...
public:
  ...
  RingoTable(const char *fname) {
    ...
    for (; Ss.Next(); ) {
      TPt<PVec<TInt> > row = PVec<TInt>::New();
      for (int i=0; i<Ss.GetFlds(); i++) {
        row->Add(Ss.GetInt(i));
      }
      Add(row);
  ...
  RingoTable Join(TStr Col1, RingoTable Table, TStr Col2) {
    ...
    // Hash all rows
    THash<TInt,TVec<TPt<PVec<TInt> > > > Hash;
    for (TIter Row = BegI(); Row<EndI(); Row++) {
      TInt Val = (*Row)->GetVal(Idx1);
      TVec<TPt<PVec<TInt> > > Dat;
    ...
    for (TIter Row2 = Table.BegI(); Row2 < Table.EndI(); Row2++) {
      ...
      for (TVec<TPt<PVec<TInt> > >::TIter Row1 = Dat.BegI(); Row1 < Dat.EndI(); Row1++) {
        // Add new row to the result
        TPt<PVec<TInt> > NewRow = PVec<TInt>::New();
        for (TVec<TInt>::TIter Val = (*Row1)->V.BegI(); Val < (*Row1)->V.EndI(); Val++) {
          NewRow->Add(*Val);
        }
        for (TVec<TInt>::TIter Val = (*Row2)->V.BegI(); Val < (*Row2)->V.EndI(); Val++) {
          NewRow->Add(*Val);
        }
        Result.Add(NewRow);
      }
  ...
  RingoTable GetUniquePairs(TStr Col1, TStr Col2) {
    ...
    int Cnt = 0;
    for (TVec<TPt<PVec<TInt> > >::TIter Row = BegI(); Row < EndI(); Row++) {
      TVec<TInt> Dat;
...
//    for (int i = Hash.FFirstKeyId(); Hash.FNextKeyId(i);) {
  ...
  void PrintNFirstRows(int N) {
    for (int i=0; i<N; i++) {
      TPt<PVec<TInt> > Row = GetVal(i);
      for (TVec<TInt>::TIter Val = Row->V.BegI(); Val<Row->V.EndI(); Val++) {
        printf("%d\t",(*Val)());
      }
      printf("\n");
    }
...
char *getFileName(char *buf, const char *basename, int size) {
  sprintf(buf ,DIR"%s_%d.hashed.tsv", basename, size);
  return buf;
}
...
void buildQAGraph(int size) {
  char buf[256];
  getFileName(buf, "posts", size);
  // 1) Load
  TExeTm t1;
  RingoTable Posts(buf);
...
void buildCommonCommentsGraph(int size) {
  char buf[256];
  getFileName(buf, "comments", size);
  // 1) Load
  TExeTm t1;
  RingoTable Comments(buf);
