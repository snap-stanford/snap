...
public:
  ...
  TCs& TCs::operator+=(const TCs& Cs){Val=(Val+Cs.Val)&MxMask; return *this;}
  TCs& TCs::operator+=(const char& Ch){Val=(Val+Ch)&MxMask; return *this;}
  TCs& TCs::operator+=(const int& Int){Val=(Val+Int)&MxMask; return *this;}
...
public:
  ...
  virtual bool Eof() const=0; // if end-of-file
  ...
  virtual int GetBf(const void* Bf, const int& BfL)=0;

  bool IsFastMode() const {return FastMode;}
  void SetFastMode(const bool& _FastMode){FastMode=_FastMode;}

  void LoadCs();
  void LoadBf(const void* Bf, const int& BfL){Cs+=GetBf(Bf, BfL);}
...
public:
  ...
  virtual int PutBf(const void* LBf, const int& LBfL)=0;
  ...
  void SaveBf(const void* Bf, const int& BfL){Cs+=PutBf(Bf, BfL);}
  ...
  void Save(const char* CStr, const int& CStrLen){Cs+=PutBf(CStr, CStrLen+1);}
  void Save(const char* CStr);
  void Save(TSIn& SIn, const int& BfL=-1);
  void Save(const PSIn& SIn, const int& BfL=-1){Save(*SIn, BfL);}
  void Save(const void* Bf, const int& BfL){Cs+=PutBf(Bf, BfL);}
...
public:
  TStdIn(): TSBase("Standard input"), TSIn() {}

  bool Eof() const {return feof(stdin)!=0;}
...
public:
  TStdOut(): TSBase(TSStr("Standard output")), TSOut(){}
...
public:
  ...
  bool Eof() const {return (BfC==BfL)&&(BfL<MxBfL);}
...
public:
  ...
  bool Eof() const { return feof(FileId) != 0; }
...
public:
  TMIn(const void* _Bf, const int& _BfL, const bool& TakeBf=false, const bool& _Free=true);
  TMIn(TSIn& SIn);
  ...
  //MB: Thou shall not destroy what thou did not create! (11. commmandement)
  ~TMIn(){if (Bf!=NULL&&Free){delete[] Bf;}}

  bool Eof() const {return BfC==BfL;}
...
public:
  ...
  void GetBf(void* Bf, const int& BfL);
  void PutBf(const void* Bf, const int& BfL);
  void Flush();
