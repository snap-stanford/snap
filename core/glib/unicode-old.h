...
public:
	...
	TUnicodeException(size_t srcIdx_, int srcChar_, const TStr& message_) : 
		srcIdx(srcIdx_), srcChar(srcChar_), message(message_) { }
};
...
public:
	...
	TUniCodec(TUnicodeErrorHandling errorHandling_, bool strict_, int replacementChar_, bool skipBom_) :
		errorHandling(errorHandling_), strict(strict_), replacementChar(replacementChar_), skipBom(skipBom_)
	{
	}
...
public:
	TUniCaseFolding() { }
	explicit TUniCaseFolding(TSIn& SIn) : cfCommon(SIn), cfSimple(SIn), cfFull(SIn), cfTurkic(SIn) { SIn.LoadCs(); }
...
public:
	...
	static int ToUnicode(int c) { Assert(0 <= c && c <= 255);
		for (int i = 0; i < sizeof(yuAsciiChars) / sizeof(yuAsciiChars[0]); i++)
			if (c == yuAsciiChars[i]) return uniChars[i];
		return c; }
	static int FromUnicode(int c) { 
		for (int i = 0; i < sizeof(uniChars) / sizeof(uniChars[0]); i++)
...
public:
	...
	{
		typedef TVecElt<TDestVec>::TElement TDestCh;
		if (clrDest) dest.Clr();
...
public:
	...
	TUniChInfo() : chCat(char(ucOther)), chSubCat(char(ucOtherNotAssigned & 0xff)), combClass(ccInvalid), simpleUpperCaseMapping(-1),
		simpleLowerCaseMapping(-1), simpleTitleCaseMapping(-1), decompOffset(-1), flags(0), script(-1), 
		lineBreak(LineBreak_Unknown), properties(0), propertiesX(0), nameOffset(-1) {
		InitAfterLoad(); }
