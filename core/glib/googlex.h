...
public:
  ...
  // files
  static PRSet LoadBin(const TStr& FNm){
    TFIn SIn(FNm); return Load(SIn);}
  static void SaveBin(const TStr& FNm, const PRSet& RSet);
  void SaveBin(const TStr& FNm){
    TFOut SOut(FNm); Save(SOut);}
...
extern "C" __declspec(dllimport) char* fetchWebPage(const char* Url, const char* Proxy);
#endif
...
#ifdef MIHA_BLAZ_DOTNET_PROXY
  static PRSet WebSearchProxy(const TStr& QueryStr, const int& MxHits=-1,
   const PNotify& Notify=NULL, const TStr& ProxyStr="");
#endif

  // google-news search
  static PRSet NewsSearch(const TStr& QueryStr, const PNotify& Notify=NULL);
