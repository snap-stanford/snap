...
#pragma omp parallel for schedule(dynamic) num_threads(NumThreads)
  ...
  //Learning embeddings
  if (!OutputWalks) {
    LearnEmbeddings(WalksVV, Dimensions, WinSize, Iter, Verbose, EmbeddingsHV, RandomSeedValue, NumThreads);
  }
}
...
void node2vec(PWNet& InNet, const double& ParamP, const double& ParamQ,
  ...
  node2vec(InNet, ParamP, ParamQ, Dimensions, WalkLen, NumWalks, WinSize,
   Iter, Verbose, OutputWalks, WalksVV, EmbeddingsHV, RandomSeedValue, NumThreads);
}
